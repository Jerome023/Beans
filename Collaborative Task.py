...
def open_add_plant_window(prefill=None, index=None):
    ...
    input_field = None  # "name", "age", "notes"
    running = True
    ...
    def save_or_add():
        nonlocal running
        plant_data = {"name": name, "age": age, "shade": shade, "notes": notes, "photo": photo_path}
        if index is None:
...
plant_img = pygame.transform.scale(plant_img, (200, 200)).convert_alpha()

speechbubble_img = pygame.image.load("SpeechBubble.png")
speechbubble_img = pygame.transform.scale(speechbubble_img, (100, 100)).convert_alpha()
...
buttons = [
    ...
    Button(30, 500, 300, 60, "Ask AI", gardening_tip_action),
]


plant_button_start_y = 70
...
while run:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            mx, my = event.pos
            # Focus AI input box
            input_box_rect = pygame.Rect(20, 450, 320, 30)
    ...
    # Draw Images
    screen.blit(plant_img, (150, 250))
    screen.blit(speechbubble_img, (50, 250))

    # Plant list
    for i, plant in enumerate(plants):
        rect = pygame.Rect(30, plant_button_start_y + i * 50, 200, 40)
        pygame.draw.rect(screen, (180, 220, 180), rect, border_radius=6)
        pygame.draw.rect(screen, (0, 0, 0), rect, 2, border_radius=6)
        screen.blit(font.render(plant["name"], True, (0, 0, 0)), (rect.x + 10, rect.y + 10))


    # AI input box
    input_box = pygame.Rect(20, 450, 320, 30)
    ...
    # AI response box with scroll
    if ai_response:
        box_rect = pygame.Rect(20, 580, 320, 60)
